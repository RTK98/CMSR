    ...
    <div
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                <a href="myAppointments.php" class="btn btn-sm btn-outline-secondary">Appointment List</a>
            </div>
            <div class="btn-group me-2">
                <a href="../customer/viewUser.php" class="btn btn-sm btn-outline-secondary">Profile</a>
            </div>
        </div>
    </div>
    <div class="container">
            ...
            <div class="col-md">
                <div class="card">
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'add') {
                        ...
                        if (empty($messages)) {
                            $db = dbConn();
                            echo $sqlCancelReason = "INSERT INTO appointmentcanceled(AppCanceledId, Reason,AddCustomer,AddCustomerDate) VALUES ('$appointmentId','$CancelReason','$addeduser','$adddate')";
                            $resultCancelReason = $db->query($sqlCancelReason);

                            echo $sqlCancelUpdate = "UPDATE appointments SET appointmentStatus=('4') WHERE AppointmentId='$appointmentId'";
                            $db->query($sqlCancelUpdate);

                            echo $sqlCancelDelete = "DELETE FROM appointmenthandling WHERE AppointmentId='$appointmentId'";
                            $db->query($sqlCancelDelete);

                            // email body items = $_SESSION['CustomerID'];
                            $db = dbconn();
                            $sqlAppNum = "SELECT * FROM `appointments` INNER JOIN appointmentcanceled ON appointmentcanceled.AppCanceledId WHERE appointments.CustomerID='$CustomerID' ORDER BY appointmentcanceled.AppCanceledId DESC LIMIT 1;";
                            ...
                            echo $subject = "You have Canceled the Apointment" . $vehicleNumber . " " . $dateww . " " . $serviceName;
                            $body = '<!doctype html>
                <html lang="en-US">
                    <head>
                        ...
                        <div class="card-body">
                            <section class="m-1">
                                <div class="card-header">
                                    <img width="60" src="<? = SYSTEM_PATH
                                         ?>/assets/img/logo.png" title="logo" alt="logo" 
                                         styly=" 
                                         display: block;
                                         margin-left: auto;
                                         margin-right: auto;
                            ...
                            <section>
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="CancelReason" rows="3" name="CancelReason"></textarea>
                                    <div class="text-danger"><?= @$messages['error_Cancel_Reason']; ?></div>
                                </div>
                            </section>
