...
?>
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 add-customer">
    <style type="text/css">
        #passwordInput{
            width: 30%;
            display: flex;
    ...
    <div class="card" style="background-color: #e1e3ef; font-weight: bold;">
        ...
        if ($_SERVER['REQUEST_METHOD'] === "POST") {
            ...
            $messages = array();
            if ($Title == "") {
                $messages['error_Title'] = "Select the Title..!";
            }
            if (empty($FirstName)) {
...
//                     
            ...
            if (empty($messages)) {
                ...
                $db = dbconn();
                $AddDate = date('Y-m-d');
                $sql = "INSERT INTO customer(Title,UserImage,FirstName,LastName,Email,Password,HouseNo,Lane,Street,Province,District,City,Gender,NIC,Dob,Status,ContactNo,AddDate)"
                        . " VALUES('$Title', '$file_name_new', '$FirstName', '$LastName', '$Email', '$Password', '$HouseNo', '$Lane1', '$Lane2', '$ProvinceName','$DistrictName', '$CityName', '$Gender', '$NIC','$BirthDay', '1','$ContactNo', '$AddDate')";
...
$alt = "EplyoeeReg";
                ...
                if ($results) {
                    ...
                    <script>
                        Swal.fire({
                            toast: true,
                            icon: 'success',
                            title: 'Account has been Creat Succefully',
                            animation: false,
                            position: 'top-right',
                            showConfirmButton: false
                        }).then(() => {
                            
                            window.location.href = '<?= SYSTEM_PATH ?>customer/login.php'; // Redirect to success page
                        });
                    </script>
                    <?php
                }
        ...
        <form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
            <div class="card-body row g-3" >
                ...
                <h5 class='m-1'style="text-align: center; font-weight: bold;">Customer Registration Form</h5>
