...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" && @$action == 'update') {
        ...
        //data clean
        $questionanswer = cleanInput($questionanswer);
        $questionname = cleanInput($questionname);
        $messages = array();
        ...
        if (empty($messages)) {
            ...
            $adddate = date('Y-m-d');
            $questionanswer = strtolower($questionanswer);
            $sqlfaqupdate = "INSERT INTO frequentasked(questionname,questionanswer,status,adduser,adddate) VALUES ('$questionname','$questionanswer','$status','$adduser','$adddate')";
            $db->query($sqlfaqupdate);
            ?>
            <script>
                Swal.fire({
                    title: 'Success!',
                    text: 'Frequant Asked question Added.',
                    icon: 'success',
