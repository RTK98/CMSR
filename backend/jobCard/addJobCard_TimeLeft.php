...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    <?php
    extract($_POST);
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$action == 'edit' || $_SERVER['REQUEST_METHOD'] == 'POST' && @$action == 'save') {
        $db = dbConn();
        ...
        if (empty($messages)) {
            ...
            $TaskId = $db->insert_id;
             $sql2 = "INSERT INTO tasks(Appointment_id,User_id,AppDate,TimeSlotId,AddUser,AddDate) VALUES ('$AppointmentId','$AssignTaskEmp','$AppDate','$TimeSlotStart','$AddUser','$AddDate')";
            $db->query($sql2);
            die();
    ...
    <form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST">
        <div class="card">
            ...
            <div class="card-body">
                <?php
                $db = dbconn();
                 $sql = "SELECT UserId,FirstName,LastName,UserRole from users LEFT JOIN attendance ON users.UserId=attendance.User_Id WHERE users.UserRole='technician' AND attendance.Status=1;";
                $result = $db->query($sql);
                $row = $result->fetch_assoc();
//                var_dump($row);
//                die();
...
//                                   var_dump($row123);
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col">
                                                    <?php
                                                    $usertechid = $row11["UserId"];
                                                    $sqlTask = "SELECT * FROM tasks WHERE User_id='$usertechid'";
                                                        ...
                                                        while ($rowTask = $resultTask->fetch_assoc()) {
                                                            ...
                                                            <div class = "alert alert-primary m-1" style="color:black;font-weight: bold;">
                                                                ...
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col-sm">
                                                                        <?php
                                                                        $endTimeSlot = $rowAppNo['TimeSlotStart'];
                                                                        $sqlAppTimeSlot = "SELECT * FROM timeslots WHERE TimeSlotId='$endTimeSlot'";
                                                                        $resultAppTimeSlot = $db->query($sqlAppTimeSlot);
                                                                        $rowAppTimeSlot = $resultAppTimeSlot->fetch_assoc();

                                                                        $endTime = $rowAppNo['AppDate'] . $rowAppTimeSlot['TimeSlotEnd'];
                                                                        ...
                                                                        <h5 class="btn btn-danger"><strong>Time Left : <?= $remainingTime ?></strong></h5>
                                                                    </div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col-sm">
