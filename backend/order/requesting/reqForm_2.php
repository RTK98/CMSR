            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="col-md-12">
                        <?php
                        extract($_POST);
                        if ($_SERVER['REQUEST_METHOD'] === "POST" && (@$action == 'edit' || @$action == 'addProduct' || @$action == 'removeProduct' || @$action == 'removeProduct1' || @$action == 'addOrder')) {
//                            
                            $jobCardId;
                            @$AppointmentId;
                            @$InspectionNo;
                            ...
                            if (!empty($InspectionNo)) {
                                ...
                                echo $sql1 = "SELECT id,JobCardNo FROM Job_cards WHERE id = '$jobCardId'";
                                $result1 = $db->query($sql1);
                                $row1 = $result1->fetch_assoc();
                                echo $JobCardNo = $row1['JobCardNo'];
                            }
...
//                                print_r($jobCardId);die();
                        ...
                        if ($_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'addProduct') {
                            ...
                            if (!empty($AppointmentId)) {
                                ...
                                if (!empty($ProductName)) {
                                    $db = dbconn();
                                    echo $sql1 = "SELECT * FROM products WHERE ProductId='$ProductName'";
                                    $result1 = $db->query($sql1);
...
//                        print_r($messages);
                        ...
                        if ($_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'removeProduct1') {
                            $ItemId;
                            $db = dbConn();
                            echo $sqlDeleteItems1 = "DELETE FROM orderrequestitems WHERE orderReqItemId='$ItemId'";
                            $db->query($sqlDeleteItems1);
                        }
                        ?>
                        <?php
                        extract($_POST);
...
//                            die();
                            ...
                            if ($resultAddproduct->num_rows > 0) {
                                while ($rowOderProduct = $resultAddproduct->fetch_assoc()) {
                                    ...
                                    echo $sqlInserDel = "DELETE FROM orderrequestitems WHERE JobCardId='$jobCardId'";
                                            ...
                                            <div class="container m-1">
                                                <section>
                                                    ...
                                                    <table class="table m-1">
                                                        ...
                                                        <tbody>
                                                            <?php
                                                            $sqlshowproduct = "SELECT odr.orderReqItemId,odr.JobCardId, odr.Qty, jb.JobCardNo, p.ProductName "
                                                                    . "FROM orderrequestitems odr "
                                                                    . "LEFT JOIN job_cardsrepair jb ON odr.JobCardId = jb.jobCardsRepairId "
                                                                    . "LEFT JOIN products p ON odr.ReqProductId = p.ProductId WHERE odr.JobCardId = '$jobCardId';";
                                                            $db = dbConn();
                                                            ...
                                                            if ($resultshowproduct->num_rows > 0) {
                                                                ...
                                                                while ($rowshowp = $resultshowproduct->fetch_assoc()) {
                                                                    ?><tr>
                                                                        ...
                                                                        <td> 
                                                                            <?php
                                                                            $ItemId = $rowshowp['orderReqItemId'];
                                                                            ?>
                                                                            <input type="text" name="ItemId" value="<?= $ItemId ?>" >
                                                                            <input type="text" name="InspectionNo<?= $ItemId ?>" value="<?= $InspectionNo ?>" >
                                                                            <input type="text" name="jobCardId<?= $ItemId ?>" value="<?= $jobCardId ?>" >
                                                            <!--                                                                            <input type="text" name="ItemId" value="<?= $ItemId ?>" >
