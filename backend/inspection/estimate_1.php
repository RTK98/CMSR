        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <?php
                    extract($_POST);
                    if ($_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'add' || $_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'addproduct' || $_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'addrepair' || $_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'remove' || $_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'removeRepair') {
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'addproduct') {
                        ...
                        if (!empty($nameproduct)) {
                            $db = dbconn();
                            $sql = "SELECT * FROM inspectionsestimateitems WHERE addedproductid='$nameproduct' AND estimateinspectionid='$InspectionId'";
                            $result = $db->query($sql);
                        ...
                        if (empty($messages)) {
                             $sqladdproduct = "INSERT INTO inspectionsestimateitems(estimateinspectionid, addedproductid,Qty,addeduser, addeddate) VALUES ('$InspectionId','$nameproduct','$QtyProduct','$addeduser','$adddate')";
                            $db = dbConn();
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === "POST" && @$action == 'removeRepair') {
                        echo $EstimatedId;
                        $db = dbConn();
                         $sqlDeleteItems = "DELETE FROM inspectionsestimateitemsrepair WHERE insEstimatedRepairId='$EstimatedId'";
                        $db->query($sqlDeleteItems);
                    }
                    ?>
                    <?php
                                                ...
                                                if ($resultshowproduct->num_rows > 0) {
                                                    ...
                                                    while ($rowshowp = $resultshowproduct->fetch_assoc()) {
                                                        ?><tr>
                                                            ...
                                                            <td> 
                                                                <input type="hidden" name="EstimatedId" value="<?= $rowshowp['estimateitemid'] ?>" >
...
//                    var_dump($rowRepair);
                        ...
                        <div class="card-body text-center">
                            ...
                            <div class="mb-3">
                                ...
                                <select for="ProductName" name="nameproduct" class="form-select" aria-label="Default select example">
                                    <option>--</option>
                                    <?php
                                    if ($resultProducts->num_rows > 0) {
                                        while ($rowProduct = $resultProducts->fetch_assoc()) {
                                            ?>
                                            <option value="<?= $rowProduct['ProductId'] ?>" <?php
                                            if (@$VehicleType == $rowProduct['ProductName']) {
                                                echo "selected";
                                            }
                    ...
                    <form action="estimate.php" method="POST" >
                        <div class="card-body text-center ">
                            ...
                            <div class="mb-3">
                                ...
                                <select for="RepairName" name="RepairName" class="form-select" aria-label="Default select example">
